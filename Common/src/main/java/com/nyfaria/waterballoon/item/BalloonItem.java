...
public class BalloonItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ThrownBalloon balloonEntity = new ThrownBalloon(player, level);
            balloonEntity.setItem(stackInHand);
            balloonEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
